...
class ResolutionBranch(nn.Module):
    def __init__(self, 
        ...
        self.patch_norm = nn.BatchNorm2d(self.channel_out)
        ...
        self.mixer2 = Mixer(input_seq = self.patch_num, 
                            ...
                            tfactor = self.tfactor, 
                            dfactor = self.dfactor)
        self.norm = nn.BatchNorm2d(self.channel_out)
        ...
        self.revin = RevIN(self.channel_out)
        # self.channelProjection = nn.Linear(self.channel_in, self.channel_out)
        self.channelProjection = nn.Sequential(nn.Linear(self.channel_in, self.channel_out * 2),
                                                nn.GELU(),
                                                nn.Linear(self.channel_out * 2, self.channel_out),
                                                )
    ...
    def do_patching(self, x):
        ...
        if padding == 'end':
            ...
            x_front = x[:, :, :1]
            x_padding = x_front.repeat(1, 1, self.patch_stride)
            x_new = torch.cat((x_padding, x), dim = -1)
